  ...
  <div class="w-full flex flex-col gap-2rem items-center layout-padding py-1.5rem lg:py-3rem">
    ...
    <div class="max-width flex flex-col gap-1rem">
      <template v-if="!isLoading">
        <template v-if="!proceedings?.transformedData || !proceedings?.transformedData.every(item => item.translations?.title)">
          <div
        ...
        <template v-else>
          <template
            v-for="item in proceedings?.transformedData"
            :key="item.id"
          >
            <nuxt-link
...
const listMeta = computed(() => {
  if (proceedings.value?.originalData?.meta.filter_count && typeof Number.parseInt(proceedings.value?.originalData.meta.filter_count) === 'number') {
