  ...
  <div class="w-full flex flex-col gap-2rem items-center layout-padding py-1.5rem lg:py-3rem">
    <div class="flex max-width">
      <base-breadcrumbs
        :bread-list="[
          ...
          {
            name: t('bibliography.technicalReports'),
            route: {
    ...
    <div class="max-width flex flex-col gap-2rem">
      <h1 class="text-2xl font-bold text-primary">
        {{ t('bibliography.technicalReports') }}
      </h1>
    </div>
...
const { data: bibliography, refresh: refreshBibliography } = useLazyAsyncData('bibliography-technical-reports', async () => {
  ...
  if (!bibliographyToken.value) {
    ...
    const [getDataError, getDataResult] = await to(authStore.fetchTechnicalReports())
    if (getDataError) {
      isLoading.value = false
      $q.notify({
...
function openLoginDialog() {
  return new Promise<TechnicalReports>((resolve, reject) => {
    dialog.value = $q.dialog({
      component: BaseLoginDialog,
      componentProps: {
        target: 'Bibliography',
        onGetBibliographyData(data: TechnicalReports) {
          resolve(data)
        },
      },
    })
...
useSeoMeta({
  title: t('bibliography.technicalReports'),
  description: t('bibliography.technicalReports'),
  keywords: t('bibliography.technicalReports'),
  ogTitle: t('bibliography.technicalReports'),
  ogDescription: t('bibliography.technicalReports'),
})
</script>
