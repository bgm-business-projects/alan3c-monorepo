  ...
  <div class="w-full flex flex-col gap-2rem items-center layout-padding py-1.5rem lg:py-3rem">
    <div class="flex max-width">
      <base-breadcrumbs
        :bread-list="[
          ...
          {
            name: t('bibliography.localJournalPapers'),
            route: {
    ...
    <div class="max-width flex flex-col gap-2rem">
      <h1 class="text-2xl font-bold text-primary">
        {{ t('bibliography.localJournalPapers') }}
      </h1>
    </div>
...
import ListItem from '../../../components/local-journal-papers/list-item.vue'
...
const { data: bibliography, refresh: refreshBibliography } = useLazyAsyncData('bibliography-local-journal-papers', async () => {
  ...
  if (!bibliographyToken.value) {
    ...
    const [getDataError, getDataResult] = await to(authStore.fetchLocalJournalPapers())
    if (getDataError) {
      isLoading.value = false
      $q.notify({
...
function openLoginDialog() {
  return new Promise<LocalJournalPapers>((resolve, reject) => {
    dialog.value = $q.dialog({
      component: BaseLoginDialog,
      componentProps: {
        target: 'Bibliography',
        onGetBibliographyData(data: LocalJournalPapers) {
          resolve(data)
        },
      },
    })
...
useSeoMeta({
  title: t('bibliography.localJournalPapers'),
  description: t('bibliography.localJournalPapers'),
  keywords: t('bibliography.localJournalPapers'),
  ogTitle: t('bibliography.localJournalPapers'),
  ogDescription: t('bibliography.localJournalPapers'),
})
</script>
