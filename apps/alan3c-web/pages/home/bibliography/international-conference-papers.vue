    ...
    <div class="flex max-width">
      <base-breadcrumbs
        :bread-list="[
          ...
          {
            name: t('bibliography.internationalConferencePapers'),
            route: {
...
import type { InternationalConferencePapers } from '~/contract/bibliography/international-conference-papers/international-conference-papers.type'
import BaseLoginDialog from '../../../components/base-login-dialog.vue'
import ListItem from '../../../components/international-conference-papers/list-item.vue'
...
const { data: bibliography, refresh: refreshBibliography } = useLazyAsyncData('bibliography-international-conference-papers', async () => {
  ...
  if (!bibliographyToken.value) {
    ...
    const [getDataError, getDataResult] = await to(authStore.fetchInternationalConferencePapers())
    if (getDataError) {
      isLoading.value = false
      $q.notify({
...
function openLoginDialog() {
  return new Promise<InternationalConferencePapers>((resolve, reject) => {
    dialog.value = $q.dialog({
      component: BaseLoginDialog,
      componentProps: {
        target: 'Bibliography',
        onGetBibliographyData(data: InternationalConferencePapers) {
          resolve(data)
        },
      },
    })
...
useSeoMeta({
  title: t('bibliography.internationalConferencePapers'),
  description: t('bibliography.internationalConferencePapers'),
  keywords: t('bibliography.internationalConferencePapers'),
  ogTitle: t('bibliography.internationalConferencePapers'),
  ogDescription: t('bibliography.internationalConferencePapers'),
})
</script>
